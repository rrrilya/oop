...
Money::Money() { // реализаци€ конструктора по умолчанию
    rubles = 0;
    kopeks = 0;
}
...
Money::Money(const Money& other) { // реализация конструктора копировани€
    rubles = other.rubles;
    kopeks = other.kopeks;
}

long Money::getRubles() const { // реализация метода получени€ рублей
    return rubles;
}

int Money::getKopeks() const { // реализация метода получени€ копеек
    return kopeks;
}
